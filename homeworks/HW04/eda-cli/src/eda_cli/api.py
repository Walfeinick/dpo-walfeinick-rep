...
async def quality_flags_from_csv(
    ...
    """
    - принимает CSV
    - использует summarize_dataset / missing_table / compute_quality_flags
    - возвращает все флаги качества в булевом виде
    """
    ...
    flags_bool: dict[str, bool] = {
        ...
        "has_constant_columns": flags_all.get("constant_columns", 0) > 0,
        "has_suspicious_id_duplicates": flags_all.get("duplicate_ids", 0) > 0,
        "has_many_zero_values": flags_all.get("zero_value_columns", 0) > 0,
    }
...
@app.post(
    "/head",
    tags=["data"],
    summary="Возвращает первые n строк CSV"
)
async def head_csv(file: UploadFile = File(...), n: int = Query(5, ge=1)):
    """
    Эндпоинт возвращает первые n строк CSV.
    Параметр n задаётся query-параметром, default=5.
    """
    if file.content_type not in ("text/csv","application/vnd.ms-excel","application/octet-stream"):
        raise HTTPException(status_code=400, detail="Ожидается CSV-файл")
